...
- [ ] **⚠️ 正式公開はオーナーの最終承認待ち**: ページ内容は更新済みだが、
  正式公開（footerリンク追加・noindex解除・robots.txt解除）はまだ実施していない。
  進める場合は実施前に必ず提案する。

### 正式公開に進める場合に必要な作業（オーナー承認後）

`/legal/commercial-transaction` を正式公開する際は、以下を順に実施する
（詳細手順: [PRODUCTION_MONITORING.md §12-5](PRODUCTION_MONITORING.md)）。

- [x] ~~プレースホルダーを実際の運営者情報に置き換える~~ → 2026-07-07完了
  （販売事業者名・運営責任者は実名、所在地・電話番号は請求時開示方針の文言で決着）
- [ ] ページ上部の「最終確認中」警告バナーを削除する
...
- [x] ~~`test:legal-trust-pages` のステップ9のアサーション（プレースホルダー存在・
  非リンク前提）を実態に合わせて更新する~~ → 2026-07-07完了
  （運営者情報の実名記載・開示方針文言の存在を検証するよう更新済み。
  非リンク・noindexの検証は変更なし）
...
- [x] ~~`/legal/commercial-transaction`公開に必要な運営者情報（販売事業者名・
  ...
  ページ内容には反映済みだが、**正式公開（footerリンク追加・noindex解除・
  robots.txt解除）はオーナーの最終承認待ちのまま**（実施前に必ず提案する）
