...
- [x] **`/legal/commercial-transaction`（特定商取引法表記に相当するページ）**:
  **2026-07-07、オーナー承認のもと正式公開した**。
  - `metadata.robots`のnoindex指定を解除（通常ページと同じくインデックス許可）
  - `public/robots.txt`の`Disallow: /legal`を削除（クロール許可）
  - ページ上部の警告バナー（「最終確認中」「社内確認用ドラフト」等）を削除
  - footer（トップページ）・`/premium`の下部リンク・`/contact`の「その他の
    リンク」・`/terms`「5. 広告・課金」の各所に「特定商取引法に基づく表記」
    リンクを追加
  - `src/app/sitemap.ts`に`/legal/commercial-transaction`を追加
    （`priority: 0.3`・`changeFrequency: "yearly"`、`/privacy`/`/terms`/
    `/contact`と同じ扱い）
  - 価格・支払方法・解約方法は `/terms` と整合する確定情報を記載済み
  - **販売事業者名・運営責任者**: オーナーから提供された実名（佐藤 慶音）を記載
  - **所在地・電話番号**: 実値は非公開のまま。「所在地および電話番号については、
    請求があった場合、法令に基づき遅滞なく開示します。開示を希望される場合は、
    お問い合わせフォームよりご連絡ください。」という開示方針の文言と
    `/contact`への導線を表示する形にした（住所・電話番号そのものは推測・
    捏造していない。正式公開後も変更していない）
  - **メールアドレス**: 既存の`SUPPORT_EMAIL`をそのまま使用（変更なし）
- [ ] **⚠️ 公開後の法律要件確認**: 所在地・電話番号の請求時開示方式が
  特定商取引法上どこまで認められるかについて、本ドキュメントでは法的な断定を
  行っていない。必要であれば専門家（行政書士・弁護士等）に確認することを推奨する。

### 正式公開作業ログ（2026-07-07実施済み）

`/legal/commercial-transaction`の正式公開にあたり、以下を実施した
（詳細手順: [PRODUCTION_MONITORING.md §12-5](PRODUCTION_MONITORING.md)）。

- [x] プレースホルダーを実際の運営者情報に置き換える → 完了
  （販売事業者名・運営責任者は実名、所在地・電話番号は請求時開示方針の文言で決着）
- [x] ページ上部の「最終確認中」警告バナーを削除する → 完了
- [x] `metadata.robots`を通常の索引可能設定に戻す → 完了（`robots`指定自体を削除）
- [x] footerに `/legal/commercial-transaction` へのリンクを追加する → 完了
  （footer・`/premium`・`/contact`・`/terms`の4箇所に追加）
- [x] `robots.txt` の `Disallow: /legal` を削除する → 完了
- [x] `sitemap.xml`に追加する → 完了
- [x] `test:legal-trust-pages` のステップ9のアサーション（プレースホルダー存在・
  非リンク前提）を実態に合わせて更新する → 完了
  （index許可・robots.txt非ブロック・footer等からのリンク存在・sitemap収録を
  検証するよう更新し、全項目PASSを確認）
...
- [x] ~~`/legal/commercial-transaction`公開に必要な運営者情報（販売事業者名・
  運営責任者名・所在地・電話番号・メールアドレス・住所/電話番号の公開方針）の
  提供~~ → **2026-07-07完了**。販売事業者名・運営責任者は実名、所在地・電話番号は
  「請求があった場合、法令に基づき遅滞なく開示する」旨と`/contact`導線で決着。
- [x] ~~`/legal/commercial-transaction`の正式公開（footerリンク追加・noindex解除・
  robots.txt解除）~~ → **2026-07-07、オーナー承認のもと正式公開完了**。
  footer・`/premium`・`/contact`・`/terms`からリンク、sitemap.xmlにも追加済み
- [ ] `/legal/commercial-transaction`の公開時の法律要件確認（所在地・電話番号の
  請求時開示方式が特定商取引法上どこまで認められるか等、専門家判断が必要な場合）
