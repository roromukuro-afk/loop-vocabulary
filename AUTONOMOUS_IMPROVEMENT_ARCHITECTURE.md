...
```
観測 → 課題発見 → 原因分析 → 改善仮説 → 実装計画 → コード修正 → 自動テスト
→ Draft PR → 人間承認 → 本番反映 → 効果測定 → 学習
```

**現状の自動化範囲(2026-07-19時点)**: 「コード修正」以外は自動化済み。コード修正そのものは
人間・Claude Codeの対話的セッション、または`scripts/improvement/patch-agent.mjs`(決定的で
小規模な修正カテゴリのみを扱う別工程)が担い、`claim-and-run.mjs`/`engineering-agent.mjs`は
コードを一切生成・変更しない(「その先の品質ゲート→自己レビュー→Draft PR作成」のみを自動化する)。
「承認済みIssueから自動でコード修正まで行われる」という意味ではないので、対外的な説明では
区別すること。詳細はAUTONOMY_LEVEL_POLICY.md「現在の自動化範囲の内訳」を参照。
