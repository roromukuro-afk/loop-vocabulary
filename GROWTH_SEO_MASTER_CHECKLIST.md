...
| ID | 施策 | 現状 | ステータス |
|---|---|---|---|
| AC-01 | aria属性・role属性 | 初回調査時点(未着手)では`aria-`使用は全体で6箇所のみ・`role=`属性は0件だった。第1弾(中核学習フロー)〜第6弾(検索・貼り付け・問い合わせフォーム、PR #62)まで修正済み。第7弾(単語獲得コアフロー、PR #63)・第8弾(AI弱点分析・単語抽出エラー、PR #64)で`aria-live`(`role="alert"`/`role="status"`)対応を実施(いずれも本番確認済み)。第9弾(`ClaimDailyTicketButton`・`ExtractWordsClient`保存成功・`CsvImportPanel`、成功メッセージ消失バグの構造修正込み)はコード実装・ローカル検証まで完了、PR未作成・マージはユーザー確認後(下記項目28参照)。第3弾の再調査で発見した追加9ファイルは第4〜6弾で全て対応完了。残り: `ContactForm`・`DeleteAccountPanel`・`IndexNowSyncButton`等、`aria-live`候補ファイル群の一部は未着手 | **一部完了(中核学習フロー・独自モーダル・フォームラベル全件、本番確認済み)・aria-liveは進行中(第7・8弾は本番確認済み、第9弾はマージ待ち)・残り未着手分あり** |
...
28. **AC-01(aria/role属性の低カバレッジ)第9弾: 非同期処理結果の`aria-live`不足・第3弾(成功メッセージ消失バグの構造修正込み)** — **コード実装・ローカル検証まで完了(PR未作成、マージはユーザー確認後)**。第8弾で切り出した2件の既存バグ(`task_5ba00fd1`・`task_4affc92d`)を本PRで実際に修正し、`CsvImportPanel.tsx`のライブリージョン対応も併せて実施:
    - `src/components/dashboard/ClaimDailyTicketButton.tsx`: `handleClaim()`成功時に`setClaimed(true)`と`setMessage(...)`を同じバッチで呼んでおり、claimedブランチへの切り替えでmessage表示div自体がアンマウントされ、成功/重複メッセージが誰にも表示されていなかった(`task_5ba00fd1`)。`statusMessage`(成功/情報用)と`errorMessage`(エラー用)を分離し、claimedブランチの表示自体に`statusMessage ?? "✅ 本日の達成は記録済みです"`を埋め込むことで、状態遷移後もメッセージが可視要素に残るよう構造を修正。加えて、常時マウント済みの`sr-only`な`role="status"`/`role="alert"`領域をclaimed/locked/active間で共有できるよう、コンポーネント全体をFragmentで包み、条件分岐の外側に置いた。可視のエラーメッセージ要素にも`role="alert"`を追加。
    ...
    - **新規E2E**: `scripts/testing/e2e/a11y-async-success-messages.mjs`(`npm run test:a11y-async-success-messages`、`run-e2e.mjs`にも登録)を新設。外部AI呼び出し(`/api/ai/extract-words`)や意図的なエラー・重複状態は全て`page.route()`による決定論的な固定レスポンスへ差し替え、スキップによる成功扱いは一切行っていない。検証内容:
      - ClaimDailyTicketButton: 成功時・already_claimed時ともclaimed表示への切り替え後もメッセージが可視要素・`role="status"`領域の両方に残ること、エラー時は`role="alert"`で通知されclaimed表示へは切り替わらないこと
      ...
      - 3回連続実行して27/27アサーション成功・skip 0件を確認。実行のたびTEST_プレフィックス付き単語帳・学習データを削除し、`is_premium`を元の値へ復元(冪等性確保)。実行後にDBを直接確認し、本テスト由来の残留データが無いこと・`is_premium`が正しく復元されていることを確認済み。
    ...
    - **別途対応**: `OnboardingModal.tsx`の`profiles.exam_goal`/`level`列が存在しないバグ(第2弾で発見、本タスクとは無関係)は、セッション内の一時的な`spawn_task`追跡だけでは永続性が無いため、[GitHub Issue #65](https://github.com/roromukuro-afk/loop-vocabulary/issues/65)として起票し直した。本PRにはコード修正を含めていない。
    - **残り(次ラウンド候補)**: `src/app/contact/ContactForm.tsx`・`src/components/account/DeleteAccountPanel.tsx`・`src/app/admin/indexnow/IndexNowSyncButton.tsx`、および優先度が低いその他約15ファイル(`JoinConsentClient`・`PromoteTeacherButton`・`CreateClassForm`・`InviteCodeManager`・`PdfTestBuilder`・`DisplayNameForm`・`PremiumCheckout`・`StudyPlanClient`・`AddToWordbook`・`AiPanel`・`signup`/`login`ページ・`FlashcardAiHint`・`AppAds`・管理画面各種)は未着手。
