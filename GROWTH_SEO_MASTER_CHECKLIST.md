...
29. **AC-01(aria/role属性の低カバレッジ)第10弾: ContactForm・DeleteAccountPanel・IndexNowSyncButtonの非同期フィードバック(構造的な既存バグの修正込み)** — **コード実装・ローカル検証まで完了(PR未作成、マージはユーザー確認後)**。3ファイルとも、単なる`role`属性追加だけでは済まない構造的な問題を併せて修正:
    ...
    - **新規E2E**: `scripts/testing/e2e/a11y-async-feedback-batch4.mjs`(`npm run test:a11y-async-feedback-batch4`、`run-e2e.mjs`にも登録)を新設。実メール送信・実アカウント削除リクエスト作成・実IndexNow送信は一切行わず、`page.route()`による決定論的な固定レスポンス(HTTPエラー・`network abort`・HTTP 200かつ`ok:false`を含む)へ差し替えている。検証内容:
      ...
      - いずれも`role="alert"`件数の判定はNext.js自体が挿入する`#__next-route-announcer__`を除外して行っている。3回連続実行して**40/40アサーション成功**・skip 0件を確認。
