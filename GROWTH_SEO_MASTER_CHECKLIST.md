...
| ID | 施策 | 現状 | ステータス |
|---|---|---|---|
| AC-01 | aria属性・role属性 | `aria-`使用は全体で6箇所のみ(`src/components`2件・`src/app`4件)、`role=`属性は0件。第1弾(中核学習フロー)・第2弾(独自モーダル3箇所)・第3弾の一部(`DictionarySearch`・`GuideEmailCapture`のラベル不足、PR #59・本番確認済み)を修正済み(下記参照)。第3弾の再調査で追加11箇所のラベル不足を新たに発見(次ラウンド候補)。残り: (1)上記追加11箇所、(2)非同期処理結果の`aria-live`不足、は未着手 | **一部完了(中核学習フロー・独自モーダル・フォームラベルの一部、本番確認済み)・残り未着手分あり** |
...
23. **AC-01(aria/role属性の低カバレッジ)第4弾: テスト解答欄のラベル不足** — **コード実装・ローカル検証まで完了(PR未作成)**。第3弾の再調査で発見した11箇所のうち、同種パターン(解答欄が`placeholder`のみでラベル不足)でまとまっている3ファイルを修正:
    ...
    - **検証**: `npm run typecheck`・`npm run lint`・`npm run build`成功。これら3ページ専用の既存E2Eテストは無し(`scripts/testing/e2e/quiz.mjs`は別ページが対象)。いずれも既存要素への`id`/`aria-labelledby`属性追加のみで、動作・見た目の変更は無い。
