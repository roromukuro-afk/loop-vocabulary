...
4. **アクセシビリティ(aria/role)の低カバレッジ** — 着手中。第11弾(signup/login/DisplayNameForm、PR #71)・第12弾(教室・教師管理フォーム、PR #72)完了、詳細は上記完了項目サマリ31・32番参照。残る対象(未着手): `PdfTestBuilder`・`PremiumCheckout`・`StudyPlanClient`・`AddToWordbook`・`AiPanel`・`FlashcardAiHint`・`AppAds`・管理画面系など
