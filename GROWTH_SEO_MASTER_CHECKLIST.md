...
| ID | 施策 | 現状 | ステータス |
|---|---|---|---|
| AC-01 | aria属性・role属性 | 初回調査時点(未着手)では`aria-`使用は全体で6箇所のみ・`role=`属性は0件だった。第1弾(中核学習フロー)〜第6弾(検索・貼り付け・問い合わせフォーム、PR #62)まで修正済み。第7弾(単語獲得コアフロー、PR #63)・第8弾(AI弱点分析・単語抽出エラー、PR #64)・第9弾(`ClaimDailyTicketButton`・`ExtractWordsClient`保存成功・`CsvImportPanel`、PR #66)で`aria-live`対応を実施(いずれも本番確認済み)。第10弾(`ContactForm`・`DeleteAccountPanel`・`IndexNowSyncButton`、成功メッセージ消失バグの構造修正込み)はPR #67として作成済み、コード実装・ローカル検証・レビュー対応完了、ユーザー確認後のマージ待ち(下記項目29参照)。第3弾の再調査で発見した追加9ファイルは第4〜6弾で全て対応完了。残り: 優先度の低い約15ファイル(`JoinConsentClient`・`PremiumCheckout`・`AiPanel`等)は未着手 | **一部完了(中核学習フロー・独自モーダル・フォームラベル全件、本番確認済み)・aria-liveは進行中(第7〜9弾は本番確認済み、第10弾PR #67はマージ待ち)・AC-01全体は未完了** |
...
29. **AC-01(aria/role属性の低カバレッジ)第10弾: ContactForm・DeleteAccountPanel・IndexNowSyncButtonの非同期フィードバック(構造的な既存バグの修正込み)** — **コード実装・ローカル検証・レビュー対応まで完了(PR #67、マージはユーザー確認後)**。3ファイルとも、単なる`role`属性追加だけでは済まない構造的な問題を併せて修正:
    ...
    - **AC-01全体は未完了**: 本項目29の対応後も、優先度の低い約15ファイル(`JoinConsentClient`・`PromoteTeacherButton`・`CreateClassForm`・`InviteCodeManager`・`PdfTestBuilder`・`DisplayNameForm`・`PremiumCheckout`・`StudyPlanClient`・`AddToWordbook`・`AiPanel`・`signup`/`login`ページ・`FlashcardAiHint`・`AppAds`・管理画面各種)は`aria-live`未対応のまま残っている。AC-01全体を「完了」扱いにはしていない。
