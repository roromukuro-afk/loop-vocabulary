...
| ID | 施策 | 現状 | ステータス |
|---|---|---|---|
| AC-01 | aria属性・role属性 | `aria-`使用は全体で6箇所のみ(`src/components`2件・`src/app`4件)、`role=`属性は0件。第1弾(中核学習フロー)・第2弾(独自モーダル3箇所)・第3弾の一部(`DictionarySearch`・`GuideEmailCapture`のラベル不足、PR #59・本番確認済み)・第4弾(テスト解答欄3箇所)を修正済み(下記参照)。第3弾の再調査で発見した追加11箇所のうち3箇所は第4弾で対応済み、残り8箇所(`QuickAddWord`・`CreateClassForm`・`materials`検索・`ExtractWordsClient`・`DisplayNameForm`・`ContactForm`)は未着手。加えて、(2)非同期処理結果の`aria-live`不足も未着手 | **一部完了(中核学習フロー・独自モーダル・フォームラベルの一部、本番確認済み)・残り未着手分あり(8箇所+aria-live)** |
