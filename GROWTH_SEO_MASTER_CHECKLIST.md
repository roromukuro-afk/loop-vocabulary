...
| T-11 | IndexNow実装 | 更新系全般 | **完了(本番初回送信確認済み)**(PR #32、merge `8c6fbc0`)。キーファイル・`submitUrlsToIndexNow()`・週次cron(`/api/cron/indexnow-sitemap-sync`)を実装。本番でキーファイル(`/724d6efdf17808d5069e6c8d78fa98bc9cd413ab302de6c35be0e113338da741.txt`)が200・プレーンテキスト・内容が鍵の値と完全一致することを確認済み。cronの認証(`CRON_SECRET` Bearer)が本番で正しく401を返す(未認証・誤った値)ことも確認済み。`INDEXNOW_KEY`をVercel Productionへ設定・再デプロイ済み(2026-07-29、`dpl_EzSjoxnvdLX7ScSGZtEt18xa3JDD`)。**本番初回手動送信結果(2026-07-29、`/admin/indexnow`)**: 管理API HTTPステータス200、内部結果`ok=true`、`totalUrls=170`、`submittedCount=170`、`skippedCount=0`、error無し。`submitUrlsToIndexNow()`は`res.ok`(2xx)の場合のみ`ok:true`を返す実装(ソースコードで確認済み)であり、UIの✅表示も`result.ok`の場合のみ描画されるため、IndexNow外部APIが2xxで正常受理したことを確認できる。**ただし外部APIの正確なステータス番号(200か202か)は管理画面の表示に含まれておらず未確認のまま**(「2xx成功」として記録、200/202を推測しない)。週次cronによる全URL再送信に加え、**教材(materials)のページ個別即時通知を実装済み**(本ラウンド、`/api/admin/materials`・`/api/admin/materials/[id]`新設、実際の公開可否(`is_public`かつ`license_status`承認済み)の遷移時に`after()`経由でその1URLだけを即時送信。詳細は`AI_SEARCH_AND_INDEXNOW_POLICY.md`「ページ個別の即時通知」参照)。残タスクは(1)教材以外(ガイド・辞書・無料ツール・リダイレクト、いずれも静的コンテンツで実行時書き込みイベントが無いため別方式が必要)へのページ個別即時通知、(2)Bing Webmaster Tools登録・送信状況監視、(3)IndexNow経由のインデックス・流入成果測定の3点 | **完了(コード実装・本番マージ・`INDEXNOW_KEY`設定・本番初回送信すべて確認済み)。教材のページ個別即時通知も実装済み(本番検証待ち)** | PR #32。`test:indexnow-submit`・`test:indexnow-sitemap-sync-cron`で検証。教材の即時通知は`test:materials-visibility`・`test:admin-materials-api`で検証 |
...
18. **教材のページ個別IndexNow即時通知**(本ラウンド、実装完了・マージ/本番確認は未実施): `MaterialAdminTable.tsx`のブラウザ直接Supabase書き込みを`/api/admin/materials`・`/api/admin/materials/[id]`へ移行し、実際の公開可否(`is_public`かつ`license_status`承認済み)の遷移時に`after()`経由で該当URLのみをIndexNowへ即時送信するよう実装。`test:materials-visibility`(単体)・`test:admin-materials-api`(実ログイン・実DB・UIボタン操作を含むE2E、13件)ですべて検証済み。マージ・本番デプロイ・本番での実際の即時送信発火確認は次のステップ。
...
7. **IndexNow実装** — 週次全件再送信は**完了**(PR #32、merge `8c6fbc0`、T-11クローズ)。本番でキーファイル配信・cron認証・`INDEXNOW_KEY`設定・週次全件再送信機能の本番初回送信(170 URL送信・0スキップ・`ok=true`・外部API 2xx成功、詳細はT-11参照)をすべて確認済み。**教材(materials)のページ個別即時通知も実装済み**(本ラウンド、`/api/admin/materials`関連。詳細はT-11・`AI_SEARCH_AND_INDEXNOW_POLICY.md`参照)。**残タスク**: (1) 教材以外(ガイド記事・辞書語ページ・無料ツール・URLリダイレクト)へのページ個別即時通知——これらは静的な配列・設定でgit+Vercelビルド時にのみ更新される性質のため、教材と同じサーバールートへのフック方式は使えず別方式(デプロイ時のgit diff検出等)が必要、(2) Bing Webmaster Toolsへの登録と送信状況の監視(ユーザーの手動作業)、(3) IndexNow経由のインデックス・流入成果測定(まだ実施していない)
