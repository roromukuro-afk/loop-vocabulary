...
## 7. 診断結果の共有URL/結果URLの設計（Phase 6・調査のみ、未実装）

オーナー指示により、慎重に検討すべき機能として今回は**実装せず設計のみ**行う。

### 7-1. 調査項目への回答

| 調査項目 | 回答 |
|---|---|
| 結果をDB保存するか | 実装する場合は必要。現状`/vocab-check`はクライアント側の状態のみで、結果はGA4イベントとしてしか送信されない |
| 個人情報を含まないか | 保存する場合、`variant`・`correct`・`total`・`level`・生成日時のみとし、`user_id`は**保存しない**（匿名の結果として扱う。ログイン中に受けた場合でも紐付けない） |
| URLにスコアや個人情報を直接入れないか | スコアを含めない。`/vocab-check/result/[id]`の`id`はランダムなUUID/短縮IDとし、スコアや正答率をURLパラメータに含めない（推測・改ざん防止、また同じ結果を装った偽装シェアを防ぐ意味もある） |
| 結果URLをindexさせるか | **index対象にしない。** ユーザーが1回の診断ごとに量産するページであり、内容も定型的なため、大量のnoindexページの一種として扱う（数千〜数万ページの一括index化とは別の意味で、そもそもindex化する価値がない） |
| OGP画像を生成できるか | 技術的には可能（Next.jsの`ImageResponse`等）。今回は未実装。Phase 5で作った`VocabCheckShareCard`のレイアウトをそのままOGP画像のテンプレートに転用できる設計にしてある |
| Xシェアに使えるか | 結果URLを本文に含めてシェアすることで、リンク先で同じ結果カードが見られるようにできる。現状のXシェアは「テキストのみ」で結果URLを含まないため、実装する場合はシェア文言にURLを追加する変更が必要 |

### 7-2. 想定するMVP仕様（次回実装する場合）

- ルート: `/vocab-check/result/[id]`（`id`は12桁程度のランダム英数字、推測困難な強度で十分）
- DBテーブル案: `vocab_check_results`（`id`, `variant`, `correct`, `total`, `level`, `created_at`のみ。`user_id`列を持たない）
- `generateMetadata()`で`robots: { index: false, follow: false }`を返す（`/share/[code]`と同じ方針。5章のnoindex方針と統一する）
- 結果の生成は診断完了時に匿名で1回だけ行い、`id`をURLに含めて「結果を見る」リンクとして提示する
- 有効期限は設けず（消える心配なくシェアできる方が体験として良い）、代わりに**個人情報を一切含めない設計**で対応する
- ページの中身は`VocabCheckShareCard`と同じ見た目 + 「自分も診断する」CTA

### 7-3. 実装しない理由（今回）

- 診断結果ページをDB保存する新機能は、`SHARED_WORDBOOKS_DESIGN.md`で扱った共有単語帳機能と同様、noindex方針・RLS方針・悪用対策を一つずつ詰める必要があり、拙速に実装するとオーナーが懸念する「個人情報の漏洩」「意図しない公開範囲」のリスクを抱えたまま公開してしまう可能性がある
- 現状のGA4イベント計測（`vocab_check_result_view`等）だけでも、結果の分布や利用状況の把握は可能なため、URL共有機能が無くても計測の空白期間にはならない
