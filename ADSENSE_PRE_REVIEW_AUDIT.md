...
| `/reports` | 独自レポート(準備中ハブ) | **noindex(2026-08-26変更, Issue #127・PR #128)** | 自己参照 | **×(除外)** | ○ | 不要 | 実データ・グラフを一切含まない「準備中」プレースホルダーと再判定→noindex化・sitemap除外 | なし | 高 | **対応済(PR #128)**。実データ公開時にindex化を再検討 |
