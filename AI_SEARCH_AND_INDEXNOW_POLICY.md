# Bing / IndexNow / AIクローラー 方針（Phase 8, 2026-07-28更新）

IndexNowはコード実装済み（下記「IndexNow 実装状況」参照）。`public/robots.txt` のAIクローラー個別指定については今回のラウンドでも変更しない（クローラー種別ごとの許可/拒否は影響範囲が大きく、判断材料が現時点で不十分なため。別タスク `feat/ai-crawler-policy-llms-txt` が並行して担当中）。
...
- **IndexNow**: 2026-07-28に実装済み。詳細は下記「IndexNow 実装状況（2026-07-28更新）」を参照。
- **robots.txt でのAIクローラー個別指定**: 現状、`OAI-SearchBot` `GPTBot` `ClaudeBot` `PerplexityBot` 等の個別User-agent行は一切存在しない（`User-agent: *` の一律ルールのみ）。つまり現時点ではすべてのAIクローラーに対して他の検索エンジンと同じDisallowルールが適用されている。（このセクションは別タスク `feat/ai-crawler-policy-llms-txt` が担当中のため、本ラウンドでは変更しない。）
...
（旧: 「sitemap.tsが約167件のみでIndexNow導入の規模的必然性が低い」という理由でIndexNowを先送りしていたが、実装コスト自体は低くURL件数と無関係に導入できると判断し2026-07-28に実装した。詳細は次のセクション。）
