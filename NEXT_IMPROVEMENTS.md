...
2b. **既存31教材の品詞(pos)補完（dry-run完了・実補完は承認待ち）**
   pos未設定9,997件を対象に、辞書API等の外部サービスは使わず、①同じword+同じmeaningが
   他教材で既にpos設定済み、②代名詞・前置詞・接続詞・冠詞の固定辞書、③数詞・曜日・月・
   誤判定リスクの低い基本副詞の固定辞書、④meaningが「〜する」→動詞、⑤meaningが「〜な/〜の」
   →形容詞、の5ルールのみを「自動補完候補（高信頼度）」とする方式で分類した
   （[MATERIALS_POS_AUDIT.md](MATERIALS_POS_AUDIT.md)参照）。
   **自動補完候補: 3,267件**（同じwordが他教材に存在し意味は問わず品詞が一貫している
   追加提案ルールは2,797件あるが、意味によって品詞が変わりうるためデフォルトの補完対象には
   含めていない）。**慎重に扱う: 6,730件**（複数品詞の可能性1,952件のうち345件が今回の
   pos補完対象と重複関連・熟語句動詞1,888件・meaning短すぎ100件・判断材料なし1,600件等）。
   補完しても新たな完全重複を作らないことを事前検証済み（0件）。
   dry-run計画・教材別内訳・ロールバックSQLは[reports/materials-pos-fill-plan.md](reports/materials-pos-fill-plan.md)参照。
   **`npm run materials:pos:apply`（要`CONFIRM_MATERIALS_POS_FILL=yes`）による実補完は
...
4. **既存教材の品詞(pos)未設定9,997件のうち3,267件の自動補完可否を判断する**（優先度B-2b）。
   dry-run・補完計画・ロールバックSQLまで準備完了（[reports/materials-pos-fill-plan.md](reports/materials-pos-fill-plan.md)）。
   実データの補完は別途明示的な承認を得てから着手する方針のため、ここでは提案のみに留める。
