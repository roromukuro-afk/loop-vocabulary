...
>
> **2026-07-05時点: 優先度A（下記29項目）はすべて完了。現在は次の優先度整理フェーズ。**
...
復習10語達成・苦手単語を復習・7日連続達成の4種を「チケット」風に可視化した。詳細は優先度A
完了項目29参照。**残課題**: `reward_tickets`テーブルへの実際の付与（消費型チケット）は
SSR描画中の二重付与リスクを避けるため今回は見送り、表示のみに留めている。実際に付与する
場合は専用の「受け取る」ボタン+サーバーアクション（1日1回のみ付与できるガード付き）の設計が
必要（末尾「ゲーミフィケーション×リワードチケットの次の一手」参照）。
...
12. ✅ **完了（2026-07-05）: ゲーミフィケーション×リワードチケット連携「今日の達成チケット」**（収益化監査#5）
   ...
   の4種を「チケット」風に表示。実際の`reward_tickets`テーブルへの付与はSSR描画中の
   二重付与リスクを避けるため今回は見送り、チケット風UI+進捗表示に留めた（残課題参照）。
   詳細は完了項目29参照。
...
## ゲーミフィケーション×リワードチケットの次の一手（優先度A完了項目29の延長・提案のみ）

「今日の達成チケット」は今回、表示専用（`reward_tickets`への実書き込みなし）で実装した。
実際にチケットを付与する場合の設計案（**実装はせず提案に留める**、DBスキーマ変更は不要）:

- ユーザーが明示的に押す「🎟️ 受け取る」ボタンを各チケットタイルに追加し、クリック時に
  サーバーアクション/API routeで`reward_tickets`へINSERTする（既存の`watchRewardedAndGrant()`
  と同じ「ユーザー操作起点の書き込み」パターンを踏襲。SSR描画中の書き込みは避ける）
- 二重付与防止には、その日すでに同じ`kind`で付与済みかを`granted_at::date = today`条件で
  事前チェックしてからINSERTする（新規カラム不要、既存の`granted_at`列で判定可能）
- 付与する`kind`は既存の5種（`ai_generation`/`pdf_export`/`extra_review`/`weak_word_test`/
  `analysis_ticket`）のいずれかを流用するか、新しい`kind`文字列（例:
  `daily_achievement`）を追加する（`kind`は自由入力のtext列のためスキーマ変更は不要）
