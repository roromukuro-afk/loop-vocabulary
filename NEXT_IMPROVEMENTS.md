...
32. ✅ **完了（2026-07-05）: リワードチケットの使い道・表示・消費導線を整理**
    ...
    **既存チケットへの影響**: なし。`extra_review`/`ai_generation`等のUI・付与・DBは
    ...
    `verify:srs-global`、全PASS。
    **残課題**: `extra_review`（もう一周/もう10問チャレンジ）は付与のみで消費コードが
    存在しないという別の未整理問題が見つかったが、今回のスコープ外のため着手していない
    （対応するかはオーナー判断、別タスク推奨）。`pdf_export`/`weak_word_test`/
    `analysis_ticket`の3種は完全に未実装のままで、実装するか型定義から削除するかは
    今後の判断が必要。`daily_achievement`への将来の交換機能追加も未着手（オーナー承認後に
    別タスクとして検討）。
...
## ゲーミフィケーション×リワードチケットの次の一手（優先度A完了項目30・31・32の延長・提案のみ）
...
明確化（完了項目32）まで実装済み。さらに発展させる場合の設計案（**いずれも実装は
せず提案に留める**）:
...
- **daily_achievementの将来の交換機能**: 現状は消費先を持たない「達成履歴」だが、
  ...
  実装前に必ずオーナー承認を得ること。
- **extra_reviewの消費コード整備**（完了項目32で発見した別の未整理問題）:
  `FlipCardRunner.tsx`/`ChoiceTestRunner.tsx`の「もう一周/もう10問チャレンジ」は
  広告視聴で`reward_tickets(kind=extra_review)`に付与されるが、実際には
  `used_amount`を見ずに直接`restart()`するだけで、DBの行は記録として溜まる一方
  消費されない。行儀の良い設計にするなら、残高を消費してから復習を開始する形に
  揃えるか、そもそも残高管理をやめてDBへのINSERT自体を廃止するかの判断が必要。
  現状も体験上の実害は無いため緊急度は低い。
