...
33. ✅ **完了（2026-07-05）: `extra_review`の消費コード未整備を解消**
    ...
    **used_amountの扱い**: `extra_review`は今後INSERTされないため`used_amount`という
    ...
    `verify:srs-global`、全PASS。
    **残課題**: 「もう一度」（無料・広告無し）ボタンが広告ゲート版とほぼ同じ機能を
    無料で提供している点は今回変更していない。広告視聴を実質的に必須にする・
    差別化するなどの設計変更は収益化方針に関わるため、対応する場合はオーナー承認の上で
    別タスクとして着手する。`pdf_export`/`weak_word_test`/`analysis_ticket`の3種は
    引き続き完全に未実装のまま。
...
## ゲーミフィケーション×リワードチケットの次の一手（優先度A完了項目30〜33の延長・提案のみ）
...
解消し、reward_ticketsに永続化しない「その場で完結する報酬」として整理した
（完了項目33）。さらに発展させる場合の設計案（**いずれも実装はせず提案に留める**）:
...
- **daily_achievementの将来の交換機能**: 現状は消費先を持たない「達成履歴」だが、
  ...
  実装前に必ずオーナー承認を得ること。
- **「もう一度」ボタンと広告ゲート版の差別化**（完了項目33の調査で発見）:
  `FlipCardRunner.tsx`/`ChoiceTestRunner.tsx`には、広告視聴が必要な
  「もう一周/もう10問チャレンジ」と全く同じ内容をほぼ無料・無制限に提供する
  「もう一度」ボタンが並んで存在する。広告視聴に何らかの実質的な価値
  （例: 出題数を通常より多くする、苦手単語を優先的に混ぜる等）を持たせて
  差別化する余地はあるが、収益化・広告視聴体験の設計判断に関わるため、
  対応する場合はオーナー承認の上で別タスクとして検討する。
