...
>
> **2026-07-04時点: 優先度A（下記20項目）はすべて完了。現在は次の優先度整理フェーズ。**
...
|---|---|---|---|---|
| 1 | TOEIC/ビジネス教材 | ❌ 弱い（後述） | 高（社会人流入・Premium転換） | 低〜中（教材追加のみ） |
| 2 | 復習の雪だるま対策（リカバリー） | ❌ 未実装 | 高（離脱防止） | 中 |
...
| 7 | SEO・内部リンク | 🟡 メタ情報/構造化データは強いが内部リンクが弱い | 中（SEO流入の底上げ） | 低 |
| 8 | 調べる→登録→復習→テスト導線 | ✅ 前回までにほぼ整備済み | — | — |
| 9 | 単語帳削除バグ | 🔴 本物のバグ（機能欠落） | — | 低（**本ラウンドで修正済み**、完了項目20参照） |

### 1. TOEIC/ビジネス教材（❌ 弱い）

全39教材・32,692語のうち、**TOEIC教材はわずか2件（TOEIC頻出単語800=2,500語 / 600=263語、
計2,763語）**。ビジネス英語専用の教材は**0件**（TOEIC教材の説明文中に「ビジネス語彙」という
文言があるのみ）。`/materials`のカテゴリ構成は「はじめての人におすすめ→大学受験→英検→
TOEIC・ビジネス英語→中学高校基礎→日常会話・学び直し」の順で、TOEIC・ビジネスセクションは
4番目、しかも中身が薄い。英検（11教材・7,759語）・大学受験（9教材・9,083語）と比べると
ボリュームの差が大きく、ユーザーの指摘通り**受験・英検に偏っている**ことをデータで確認した。
社会人が初回訪問時に「使えそう」と感じられる教材が量的に不足している。

**改善候補（教材追加は別タスクで実施。データ内容の作成は本ラウンドでは行っていない）**:
TOEIC 基礎100・TOEIC頻出動詞100・TOEIC頻出名詞100・ビジネス英語 基礎100・会議/メール英語100

### 2. 復習の雪だるま問題（❌ 未実装）

SRSスケジューリング自体（V1固定間隔・V2動的SM-2ライク）は正常に機能しているが、
**「サボった後の救済導線」は一切存在しない**:
- `/review`の復習キューは`.limit(50)`のハード上限のみ。ソフト上限・「今日はここまで」提案なし
- ダッシュボードに復習待ち件数が生の数字でそのまま表示される（`復習待ち: 127 単語`のように、
  溜まるほど数字だけが増えて威圧感が出るUIになっている）
- 「まず10語だけ」ボタン、復習スケジュールの一括再調整、優先度に応じた分割消化、
  「遅れても大丈夫」といった非難しないメッセージング、いずれも未実装
- 優先順位付け自体は`next_review_at`昇順（最も遅延している単語から）で機能しており、
  ここは既に妥当な設計

Anki系アプリで離脱率に直結するとされる典型的な穴であり、既存の実装済み機能
（ゲーミフィケーション・ダッシュボード）と比べて最も手薄な領域。
...
### 7. SEO・メタ情報・内部リンク（🟡 メタ情報/構造化データは強いが内部リンクが弱い）
...
実装済み。sitemapも教材・guide・grammarの動的ルートを網羅している。
**実質的な弱点は内部リンクの密度**: 教材詳細ページに「関連教材（同カテゴリ・同レベル）」への
リンクが無い、`/dictionary`から`/materials`や`/grammar`への導線が無い、`/grammar`や`/guide`
から`/materials`への逆導線が薄い、共通のBreadcrumb UIコンポーネント（表示用、構造化データとは
別）が無い。低コストで内部リンクを増やせる余地が大きい。
...
2. **復習リカバリーモードの基本実装**（収益化監査#2、離脱防止・高インパクト）
   `/review`に「まず10語だけ」等の少量スタートボタン、ダッシュボードの復習待ち表示を
   生の件数だけでなく「今日はここから」という前向きな文言に変える、といった最小構成から
   着手する。SRS V2の間隔計算ロジック自体（`ease_factor`/`interval_days`）や既存の
   `next_review_at`昇順ソートは変更しない。詳細設計は着手前に個別提案する。

3. **教材・辞書ページの内部リンク強化**（収益化監査#7、SEO流入・低コスト高効果）
   教材詳細ページに同カテゴリ・同レベルの「関連教材」リンクを追加、`/dictionary`から
   `/materials`（または関連単語の教材）への導線を追加。メタ情報・構造化データ自体は
   既に強いため、既存ページへのリンク追加のみで新規ページ作成は不要。
...
10. **TOEIC・ビジネス英語スターターパックの追加**（収益化監査#1、社会人・TOEICユーザー取込）
   TOEIC教材が2件（計2,763語）・ビジネス英語専用教材が0件という監査結果を踏まえ、
   「TOEIC 基礎100」「TOEIC頻出動詞100」「TOEIC頻出名詞100」「ビジネス英語 基礎100」
   「会議・メール英語100」等のスターターパックを追加する。**教材データの大量作成は
   本ラウンドのスコープ外のため、別タスクとして着手する**（既存のプリセットパック追加の
   手順・品質チェック基盤をそのまま流用できる）。
