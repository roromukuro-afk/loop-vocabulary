...
- [ ] `npm run test:e2e`（フルE2E一式）を実行し、6フロー（onboarding/dictionary・SRS V2・teacher・admin・教材インポート・4択出題ロジック）が全PASSか
...
| `npm run test:e2e` | **本番デプロイ前**（大きめの変更時）／**週1定期** | onboarding・SRS V2・teacher・admin・教材インポート・4択出題ロジックの6フローを実ブラウザで通しで検証 |
...
| `npm run test:quiz` | `src/lib/quiz/wordSelection.ts`を変更した時（DB不要・数秒） | 出題選定(未学習優先/due・weak重み付け/直近除外)・選択肢生成(重複なし/空欄なし/正解1つ)の単体テスト |
