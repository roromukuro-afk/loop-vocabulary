...
async function main() {
  ...
  try {
    ...
    const rows = [
      ...
      // II: tiktok / social / campII / content=postII → landing_view自体が一度も無く、
      // SNS投稿が/tools/word-list-cleanerへ直接リンクするケース。HHのexam_countdown_
      // page_viewedと全く同じ見落としパターン(PR #105実装時点でのセルフ監査で発見):
      // word_list_cleaner_page_viewedがLANDING_EVENT_NAMESに含まれていないと、この
      // セッションのlandingが0件になってしまう。
      { event_name: "traffic_source_detected", anonymous_session_id: `${prefix}ii`, source: "tiktok", campaign: "campII", path: null, user_id: null, properties: { source: "tiktok", medium: "social", content: "postII" }, occurred_at: offset(0), is_test_event: false, schema_version: 1 },
      { event_name: "word_list_cleaner_page_viewed", anonymous_session_id: `${prefix}ii`, source: "tiktok", campaign: "campII", path: "/tools/word-list-cleaner", user_id: null, properties: {}, occurred_at: offset(100), is_test_event: false, schema_version: 1 },
    ];
    ...
    if (result.socialLandingIdentities === 27) {
      ok('social landing identities合計が27(A/B/D/G/H-visit1/J/K/L/M-visit1/M-visit2/N/O/P/T-visit1/U-visit1/U-visit2/W-visit1/X-visitA/Y/Z/AA/BB/CC/EE/GG/HH/II。C=非social/E=test account/F=test event/H-visit2=非social/I=medium違いは除外、Jのreloadは1visitに畳み込み、Mの40分間隔は2visitのまま、Nの0/20/40分3連続reloadは1visitに畳み込み、Oの日付境界またぎvisitも正しく計上。Qは真のlandingがウィンドウ開始前、T-visit2はtest account紐付け、Vはウィンドウ終了直後のtest account認証紐付け、W-visit2はmedium違いによる非social判定、X-visitBはそのvisit自身のlanding行が無いため計上されない。Yは同一source+campaignで複数content並行visitのため"(ambiguous)"のcontentで1件計上される。ZはcontentZAが非active化した後のcontentZBのみが正しく計上される。AAはタブA自身のlastSeenAt延長により40分目のconversionが正しくattributionされる。BBはウィンドウ内のlandingが1件計上される(user_id付き行自体はウィンドウ外)。CCはXの15分目のハードリロードが正しく既存visitへ畳み込まれ、15.5分目の後続ページが重複landingとして水増しされない。EEは20分目の行が両方のactive候補を延長するため40分目の行も引き続き"(ambiguous)"のまま1件計上される。GGはlanding自体は1visitのまま(=vocab_test_maker_generatedを2回発火しても水増しされない)。HHはexam_countdown_page_viewedのみ(landing_viewが一度も無い)のセッションで、これもlandingとして正しく数えられる。IIはword_list_cleaner_page_viewedのみ(landing_viewが一度も無い)のセッションで、HHと同じ理由でこれもlandingとして正しく数えられる');
    } else {
      bad(`social landing identities合計が想定外: ${result.socialLandingIdentities}(期待値: 27)`);
    }
    ...
    const expectedBuckets = { x: 20, threads: 0, instagram: 1, tiktok: 1, youtube: 1, pinterest: 1, facebook: 0, line: 0, other_social: 3 };
    let bucketsOk = true;
