...
オーナー確認済み（完了）。特商法ページも運営者情報を反映のうえ、2026-07-07に
オーナー承認を得て正式公開済み。AdSense審査（審査待ち継続中）・初回実課金確認・
無効化済みWebhook endpointの削除判断など、オーナー側の確認・判断待ちの項目は
引き続き残っている。**
...
- **Search Consoleでは`/materials/toeic`がインデックス登録済み**であることを確認。
  ...
  様子見する。
- **特商法ページ（`/legal/commercial-transaction`）は2026-07-07に正式公開済み**
  （販売事業者名・運営責任者は実名、所在地・電話番号は「請求があった場合、法令に
  基づき遅滞なく開示する」旨と`/contact`導線を表示。noindex解除・robots.txt解除・
  footer/`/premium`/`/contact`/`/terms`からのリンク追加・sitemap追加を実施済み）。
- 一方で、**初回実課金の実データ確認・無効化済み重複Webhook endpointの削除要否
  判断・特商法ページ公開時の法律要件確認**は、実運用で実際に起きた時・オーナーの
  判断や専門家確認が無いと完了できない項目として引き続き残っている。これらが
  揃うまでは、「完全にリリース準備完了」と言い切ることは避ける。
...
| `/legal/commercial-transaction`（特商法表記に相当） | **2026-07-07、オーナー承認のもと正式公開**: noindex解除・`robots.txt`の`Disallow: /legal`解除・footer/`/premium`/`/contact`/`/terms`からのリンク追加・`sitemap.xml`追加を実施。販売事業者名・運営責任者は実名（佐藤 慶音）、所在地・電話番号は実値非公開のまま「請求があった場合、法令に基づき遅滞なく開示する」旨と`/contact`への導線を表示 | ✅ 完了 | 継続監視のみ | — |
| 公開時の法律要件確認 | 未実施（本ドキュメントでは法律判断を行わない。所在地・電話番号の請求時開示方式が特定商取引法上どこまで認められるかも断定していない） | 🕒 後日確認 | 必要であれば専門家（行政書士・弁護士等）に確認 | オーナー |
...
**2026-07-07更新**: 唯一の要アクション項目だった「特商法ページ正式公開」は
オーナー承認のもと本日完了した。現時点で新たな要アクション項目は無い
（下記3-2の後日確認予定の項目のみが残っている）。

### 3-2. 後日確認予定の項目（現時点で追加対応不要、監視継続のみ）

1. **AdSense審査ステータスの定点観測**（2026-07-07確認時点で`Getting ready`のまま。
...
3. **Stripe初回実課金時のWebhook deliveryとPremium反映確認**（2026-07-07時点でも
   ...
   7点を確認する）
4. **特商法ページ公開時の法律要件確認**（所在地・電話番号の請求時開示方式が
   特定商取引法上どこまで認められるかは本ドキュメントでは断定していない。
   必要であれば専門家（行政書士・弁護士等）に確認する）
...
**2026-07-07更新**: Vercel Cron・AdSense・Search Consoleの確認、特商法ページの
運営者情報反映と正式公開が完了したため、現時点で「要アクション」の項目は無い。
残るのは以下の後日確認・監視系の項目のみ。

1. **Search Consoleで`/materials/business`・`/materials/news`のインデックス状況を
   1〜2週間後に再確認する**（すでに様子見中のため、日付が来たら確認するだけでよい）
2. **AdSense審査ステータスを引き続き定点観測する**（`Getting ready`から変化するまでは
   追加対応不要。Readyになるまで広告増設もしない）
3. **実課金が発生した際にStripeの配信ログとPremium反映を確認する**（発生タイミングは
   コントロールできないため、実際に発生したら都度対応する）
4. **特商法ページ公開時の法律要件確認**（必要であれば専門家に確認する。緊急性は低い）
5. **無効化済みStripe重複Webhook endpointの取り扱いを判断する**（正規endpointでの
   初回実課金確認が前提のため、上記3の後で問題ない）
