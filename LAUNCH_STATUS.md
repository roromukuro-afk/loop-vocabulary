...
オーナー確認済み（完了）。一方で、AdSense審査（審査待ち継続中）・特定商取引法ページの
運営者情報・初回実課金確認など、オーナー側の確認・判断待ちの項目は引き続き残っている。**
...
- **Search Consoleでは`/materials/toeic`がインデックス登録済み**であることを確認。
  ...
  様子見する。
- 一方で、**特商法表記の公開・初回実課金の実データ確認・無効化済み重複Webhook
  endpointの削除要否判断**は、実運用で実際に起きた時・オーナーの情報提供や判断が
...
1. **特商法ページ正式公開に必要な運営者情報の提供**（販売事業者名・運営責任者名・
   所在地・電話番号・メールアドレス・住所/電話番号の公開方針・footer公開タイミング。
   個人情報を推測・捏造しない方針のため、提供があるまでドラフト非公開のまま。
   **これらはまだ未提供のため、現時点ではページ正式公開はしない**）
...
**2026-07-07更新**: Vercel Cron・AdSense・Search Consoleの確認が完了したため、
残るのは以下の項目。

1. **オーナーが特商法表記に必要な運営者情報を決める・提供する**（唯一の「要アクション」
   項目。対応に時間がかかる可能性があるため早めに着手するとよいが、公開自体を急ぐ
   必要は無い）
