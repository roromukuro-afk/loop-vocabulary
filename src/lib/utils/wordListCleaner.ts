...
// スキップされていた(Codexレビュー指摘対応、PR #105、6巡目、P1)。境界直後に短い
// 全角/半角丸括弧の注記(最大12文字、閉じ括弧の後の空白込み)を許容してから日本語文字を
// 要求するオプショナル部分を追加する。日本語文字が最終的に続かない行("Q&A (note)"の
// ような注記のみの英語行)には従来どおりマッチしない。
const LATIN_TO_JAPANESE_BOUNDARY =
  /[A-Za-z0-9+#./][ 　]*(?=(?:[（(][^）)]{0,12}[）)][ 　]*)?[぀-ヿ一-鿿])/;
