...
//
// さらに"100% 百パーセント"・"hello! こんにちは"・"what? 何"のように%/!/?で終わる
// 見出し語も同じ理由で一切マッチせずスキップされていた(Codexレビュー指摘対応、
// PR #105、round-17再監査、P2)。単純に「空白直前は日本語文字以外なら何でも許可」
// (否定文字クラス)へ一般化すると、"go (irregular verb) 行く"のような既知の品詞
// 注記ラベルではない括弧(意図的にnullを返し書き換えない、7巡目の既存回帰テスト
// 参照)まで閉じ括弧")"を境界として誤ってマッチしてしまうため、この許可リスト方式は
// 維持したまま、報告された3記号だけを個別に追加する。
//
