    ...
    <div className="max-w-3xl mx-auto px-5 py-10 prose prose-sm">
      ...
      <ul className="list-disc pl-5 text-sm text-navy-700">
        ...
        <li>
          ...
          Google Analytics でどのページがどれだけ利用されているか等のイベントを計測する
          ほか、Microsoft Clarity では画面内のクリック・スクロール等の操作を録画する
          ヒートマップ・セッション記録機能を利用しています。これらのツールに送信するのは
          ページの種類・操作したボタンの種類・匿名の識別子・アクセス日時等のみで、
          メールアドレス・氏名・単語帳の内容など個人を特定できる情報をイベントパラメータ
          として送信することはありません。ただし Microsoft Clarity のセッション記録機能は
          画面表示内容を録画する性質上、入力欄のマスキング（伏字化）設定は Microsoft
          Clarity 管理画面側の設定に依存します。
        </li>
      </ul>
