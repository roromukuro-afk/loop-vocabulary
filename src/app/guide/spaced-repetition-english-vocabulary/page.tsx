...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "いいえ、単語ごとに動的に計算されるため、全員・全単語共通の「◯日後」という一律のパターンはありません。まだ一度も評価していない単語は数日以内に再出題され、その後は簡単だと感じるたびに間隔が伸びていきます(上限は半年程度)。もう一度と答えると、それまでの間隔に関係なく翌日に戻ります。同じ単語でも、フラッシュカード・タイピング・4択のどのモードで答えたかによって、次の間隔の伸び方が変わります。",
  },
];
        ...
        <div className="bg-white rounded-2xl border border-navy-100 shadow-sm p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { icon: "2️⃣", title: "評価に応じて次の出題日を計算", desc: "その単語をまだ一度も評価していない場合は、学習モードに応じて1〜3日程度(簡単なら長め)の間隔からスタートします。一度でも評価した後の正解では、単語ごとに持つ「思い出しやすさ」の指標に直前の間隔を掛けて次の間隔を決め、「簡単」ではこの指標がさらに強く効いて間隔も指標も伸び、「難しい」では指標は下がり、その回の間隔は固定倍率に学習モード別の重みを掛けた抑えめな値になります(モードによっては前回とほぼ同じか、やや短くなることも)。「もう一度」を選ぶと指標に関わらず翌日に戻ります(間隔の上限は半年程度)。" },
