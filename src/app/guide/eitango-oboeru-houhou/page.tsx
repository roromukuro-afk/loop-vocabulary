...
const METHODS = [
  ...
  },
  {
    ...
    body: "一夜漬けのように詰め込んで一度に覚えるより、忘れかけたタイミングで復習を繰り返す方が長期的に記憶に残りやすい傾向があります。この「間隔を空けて復習する」考え方をもとに、SRS（間隔反復システム）は次の復習タイミングを自動で計算します。具体的には、単語ごとに「思い出しやすさ」を示す指標を持ち、「普通」と答えるとこの指標に直前の間隔を掛けて次の間隔を決めます。「簡単」ではこの指標がさらに強く効いて間隔がより伸び、「難しい」ではその回の間隔は固定倍率に学習モード別の重みを掛けた抑えめな値になります(モードによっては前回とほぼ同じか、やや短くなることもあります)。もう一度と答えた単語は指標に関わらず翌日に戻ります。単語ごと・人ごとに指標が異なるため、全員・全単語に共通する「◯日後」という固定スケジュールはありません。仕組みの詳細と毎日の組み立て方は、専用記事で解説しています。",
    tips: [
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "全員・全単語に共通する固定の「◯日後」というスケジュールはありません。Loop Vocabularyでは単語ごとに「思い出しやすさ」を示す指標を持ち、「普通」と答えると直前の間隔とこの指標をもとに次の復習日を計算します。「簡単」ではこの指標がさらに強く効いて間隔がより伸び、「難しい」ではその回の間隔は固定倍率に学習モード別の重みを掛けた抑えめな値になります(モードによっては前回とほぼ同じか、やや短くなることもあります)。目安として、初めて正解した単語は数日以内に再出題され、簡単だったと答えた単語ほど次第に間隔が伸びていきます（間隔の上限は半年程度）。逆に「もう一度」と答えた単語は、それまでの間隔に関係なく翌日に戻ります。同じ単語でも、答えた学習モードやこれまでの自己評価によって具体的な日数は変わります。",
  },
];
