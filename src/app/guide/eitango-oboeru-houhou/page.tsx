...
const METHODS = [
  ...
  },
  {
    ...
    body: "一夜漬けのように詰め込んで一度に覚えるより、忘れかけたタイミングで復習を繰り返す方が長期的に記憶に残りやすい傾向があります。この「間隔を空けて復習する」考え方をもとに、SRS（間隔反復システム）は次の復習タイミングを自動で計算します。具体的には、単語ごとに「思い出しやすさ」を示す指標を持ちます。その単語の復習間隔がまだ記録されていない場合は自己評価に応じて1〜3日程度(簡単と答えた場合のみ長め)からスタートし、間隔が一度でも記録された後の正解では「普通」ならこの指標に直前の間隔を掛けて次の間隔を決めます。「簡単」ではこの指標がさらに強く効いて間隔も指標も伸び(指標には上限があり、そこに達すると頭打ちになります)、「難しい」では指標は下がり(同様に下限で頭打ち)、その回の間隔は前回のおよそ1.2倍にとどまります(間隔が小さいうちはほぼ変わらないこともあります)。もう一度と答えた単語は指標に関わらず翌日に戻ります。単語ごと・人ごとに指標が異なるため、全員・全単語に共通する「◯日後」という固定スケジュールはありません。仕組みの詳細と毎日の組み立て方は、専用記事で解説しています。",
    tips: [
...
const MISTAKES = [
  ...
  { title: "覚えたらそれで終わり（復習しない）", fix: "SRSで復習サイクルを回す。復習のタイミングは単語ごとにSRSが自動計算するため、日数を自分で覚えておく必要はない。" },
...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "全員・全単語に共通する固定の「◯日後」というスケジュールはありません。Loop Vocabularyでは単語ごとに「思い出しやすさ」を示す指標を持ちます。その単語の復習間隔がまだ記録されていない場合は自己評価に応じて1〜3日程度(簡単と答えた場合のみ長め)からスタートし、間隔が一度でも記録された後の正解では「普通」なら直前の間隔とこの指標をもとに次の復習日を計算します。「簡単」ではこの指標がさらに強く効いて間隔も指標も伸び(指標には上限があり、そこに達すると頭打ちになります)、「難しい」ではその回の間隔は前回のおよそ1.2倍にとどまります(間隔が小さいうちはほぼ変わらないこともあります。間隔の上限は半年程度)。逆に「もう一度」と答えた単語は、それまでの間隔に関係なく翌日に戻ります。同じ単語でも、答えた学習モードやこれまでの自己評価によって具体的な日数は変わります。",
  },
];
        ...
        <div className="bg-white rounded-2xl border border-navy-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { step: "4", label: "復習スケジュールに乗せる", detail: "単語ごとにSRSが自動計算する復習タイミングに従う（間隔は単語や評価によって変わる）" },
            ].map((s) => (
