...
const FAQ_ITEMS = [
  ...
  },
  {
    ...
    a: "全員・全単語に共通する固定の「◯日後」というスケジュールはありません。Loop Vocabularyでは単語ごとに「思い出しやすさ」を示す指標を持ちます。初めて正解した単語は学習モードに応じて1〜3日程度(簡単なら長め)からスタートし、2回目以降の正解では「普通」なら直前の間隔とこの指標をもとに次の復習日を計算します。「簡単」ではこの指標がさらに強く効いて間隔も指標も伸び、「難しい」ではその回の間隔は固定倍率に学習モード別の重みを掛けた抑えめな値になります(モードによっては前回とほぼ同じか、やや短くなることも、間隔の上限は半年程度)。逆に「もう一度」と答えた単語は、それまでの間隔に関係なく翌日に戻ります。同じ単語でも、答えた学習モードやこれまでの自己評価によって具体的な日数は変わります。",
  },
];
