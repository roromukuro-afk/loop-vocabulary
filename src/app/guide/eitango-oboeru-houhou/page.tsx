...
const METHODS = [
  ...
  },
  {
    ...
    body: "一夜漬けのように詰め込んで一度に覚えるより、忘れかけたタイミングで復習を繰り返す方が長期的に記憶に残りやすい傾向があります。この「間隔を空けて復習する」考え方をもとに、SRS（間隔反復システム）は次の復習タイミングを自動で計算します。具体的には、単語ごとに「思い出しやすさ」を示す指標を持ち、「普通」と答えるとこの指標に直前の間隔を掛けて次の間隔を決めます。「簡単」ではこの指標がさらに強く効いて間隔がより伸び、「難しい」ではその回の間隔は一定倍率で緩やかに伸びるにとどまります。もう一度と答えた単語は指標に関わらず翌日に戻ります。単語ごと・人ごとに指標が異なるため、全員・全単語に共通する「◯日後」という固定スケジュールはありません。仕組みの詳細と毎日の組み立て方は、専用記事で解説しています。",
    tips: [
