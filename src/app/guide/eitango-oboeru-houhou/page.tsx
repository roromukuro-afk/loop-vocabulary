...
export const metadata: Metadata = {
  ...
  description:
    "英単語が覚えられない・すぐ忘れる悩みに、自己想起（アクティブリコール）・忘却曲線に沿った復習・語源学習・例文記憶など、研究にもとづく考え方と実践のコツを解説。英検・TOEIC・大学受験対応。",
  openGraph: {
    title: "英単語の覚え方・効率的な記憶術【自己想起×忘却曲線】",
    description: "自己想起・忘却曲線に沿った復習を中心に、英単語の覚え方を6つのアプローチで解説。",
...
const METHODS = [
  {
    ...
    body: "選択肢や日本語訳を見て「知っている」と感じる感覚（再認）と、何もない状態から自力で意味を引き出す力（想起）は別の働きです。単語帳を眺めるだけの学習は再認力しか鍛えられず、テストや読解で必要な想起力とズレが生まれます。答えを見る前に、まず自分の力で思い出そうとする練習（自己想起）に切り替えることが、多くの学習法の中でも効果が比較的大きいとされています。",
    tips: [
      "単語だけを見て、日本語訳を自分の力で思い出してから答え合わせをする",
      "「すぐ思い出せた／時間がかかった／思い出せなかった」で自己評価する",
      "思い出せなかった単語こそ、語源やAI解説で深掘りする",
      "4択は消去法で正解できてしまうため、仕上げの確認用として使う",
    ],
  },
  {
    icon: "🧠",
    ...
    body: "一夜漬けのように詰め込んで一度に覚えるより、忘れかけたタイミングで復習を繰り返す方が長期的に記憶に残りやすい傾向があります。この「間隔を空けて復習する」考え方をもとに、SRS（間隔反復システム）は次の復習タイミングを自動で計算します。仕組みの詳細と毎日の組み立て方は、専用記事で解説しています。",
    tips: [
      "毎日10〜20語ずつ新規単語を追加し、増やしすぎない",
      "アプリの「復習待ち」を優先してこなす",
  ...
  },
  {
    ...
    body: "単語単体より「文脈」の中で覚えた方が忘れにくい傾向があるという指摘があります。「persevere = 頑張る」より「She persevered through difficulties.（彼女は困難を乗り越えて頑張り続けた）」のように、具体的な文で覚えることで意味・使い方・コロケーションを同時に押さえられます。",
    tips: [
  ...
  },
  {
    ...
    body: "手を動かして書くことは、特にスペルの定着に役立つとされています。ただし「ただ写す」だけでは効果が薄く、意味を意識しながら書くこと・書いた後に隠して書けるか確認するステップが重要です。",
    tips: [
      "1語につき3〜5回程度書く（それ以上は時間対効果が下がりやすい）",
      "発音しながら書く",
...
const MISTAKES = [
  { title: "1日に覚えようとする単語が多すぎる", fix: "1日10〜20語程度に絞る。量より「思い出せる状態」を重視する。" },
  { title: "覚えたらそれで終わり（復習しない）", fix: "SRSで復習サイクルを回す。覚えた翌日・3日後・1週間後に復習する。" },
  { title: "日本語訳だけで覚えようとする", fix: "例文・語源・発音もセットで覚える。意味だけでは使い方が身につきにくい。" },
  { title: "単語帳をただ眺めるだけ（再認しか鍛えていない）", fix: "答えを隠してから自分の力で思い出す（自己想起）練習を組み込む。" },
...
const FAQ_ITEMS = [
  {
    ...
    a: "選択肢や日本語訳を見て「知っている」と感じる感覚（再認）と、何もない状態から自力で意味を引き出す力（想起）は別の働きです。単語帳を眺めるだけの学習は再認力しか鍛えられず、テストや読解で必要な想起力とズレが生まれます。",
  },
  {
...
const JSON_LD = {
  ...
  description: "自己想起・忘却曲線に沿った復習を中心に、英単語の覚え方を6つのアプローチで解説する。",
    ...
    <div className="min-h-dvh bg-[#f7f9fc] pb-16">
      ...
      <div className="max-w-2xl mx-auto px-4 -mt-4 space-y-5">
        ...
        <div className="bg-white rounded-2xl border border-navy-100 p-5">
          ...
          <div className="space-y-3">
            {[
              ...
              { step: "3", label: "仕上げの確認", detail: "4択・入力テストで最終確認（消去法で正解できてしまう点に注意）" },
        ...
        <GuideByline
          ...
          sources={
            <>
              自己想起（検索練習・testing effect）と分散学習（SRSの考え方の土台）は、いずれも認知・教育心理学の学習法研究で広く支持されている考え方です。ここでは特定の英単語学習実験ではなく、学習法全般を対象とした下記のレビュー論文を参考にしています。実際の効果には個人差があり、英単語学習に限定して検証した研究そのものではない点にご留意ください。
              <br />
              <a
              ...
              >
                Dunlosky, J., Rawson, K. A., Marsh, E. J., Nathan, M. J., &amp; Willingham, D. T. (2013). Improving Students&apos; Learning With Effective Learning Techniques. <em>Psychological Science in the Public Interest</em>, 14(1), 4-58.（Association for Psychological Science、閲覧日: 2026-07-21）
              </a>
            </>
          }
