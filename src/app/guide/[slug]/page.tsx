...
不規則動詞（原形・過去形・過去分詞形が規則的に-edで変化しない動詞）は、アルファベット順の単語帳でバラバラに覚えると定着しにくい単語です。しかし、変化の**パターンで4つに分類**すると、一気に覚えやすくなります。
...
このうち②のABB型が最も数が多く、覚える優先度が高いパターンです。
...
全部同じ型（cut, put, set等）とABA型（come, become, run）は数が少ないため、例外パターンとして個別に記憶します。

---

## まとめ

不規則動詞は、アルファベット順ではなく**変化パターン別（全部同じ・ABB型・ABA型・ABC型）**に分類して覚えるのが効率的です。特にABB型は数が多く、過去形と過去分詞が同じ形なので覚える負担を減らせます。ABC型には超頻出動詞が集中しているため、優先順位を意識して取り組みましょう。
