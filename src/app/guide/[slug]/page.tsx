...
文法は「ルール暗記」ではなく「理由の理解」で定着します。Loop の英文法レッスンでは、こうした考え方ベースの解説を例文・確認問題つきで学べます。文法と並行して、例文ごと単語を覚えると英語力が一気に伸びます。
  ...
  "eiken-2kyu-tango-nanko": {
    ...
    content: `
## 結論：英検2級合格に必要な語彙数は約5,000語

英検2級は「高校卒業程度」の英語力を測る試験で、合格に必要な語彙数は **約5,000語** とされています。ただし、これは「5,000語をすべて完璧に」という意味ではなく、**頻出する重要語を確実に押さえれば合格ラインに届く**というのが実態です。

特に語彙問題（大問1）は全体の配点に直結するため、**頻出上位1,500語を確実にする**ことが合格の最短ルートになります。
...
- **語彙問題（大問1）**：25問中、知っていれば即答できる
- **長文読解**：未知語が多いと内容が取れず時間も足りなくなる
- **ライティング・面接**：使える語彙が多いほど表現の幅が広がる

逆に言えば、**語彙を固めるだけで全セクションのスコアが底上げ**されます。文法や読解テクニックより、語彙が最もコスパの良い投資先です。
...
英検2級合格に必要な語彙は **約5,000語**、合格の鍵は**頻出上位1,500語を確実にする**ことです。語彙はすべてのセクションに効く最強の投資。基礎の穴をふさぎ、頻出語をSRSで固め、過去問で回収すれば最短で届きます。

Loop Vocabulary なら、英検2級レベルの単語帳を無料でインポートし、忘却曲線復習とAI苦手分析で効率的に語彙を固められます。今日から無料で始めましょう。
    `,
    faq: [
      { q: "英検2級の単語をすべて覚えないと合格できませんか？", a: "いいえ、すべてを完璧にする必要はありません。約5,000語が目安ですが、語彙問題で問われる頻出上位1,500語前後を確実にし、長文で大意がつかめれば合格ラインに届きます。頻出語を優先するのが効率的です。" },
...
両単語帳とも「頻度順／でる順」で並んでいるため、**冒頭の最頻出帯は収録語が大きく重なります**。実際に両者の最初の40語を見比べると、以下の単語は両方の冒頭に共通して登場します。
...
つまり「最初の数百語」はどちらを使っても覚える単語はほぼ同じです。**序盤で差はつきません**。違いが出るのは「覚え方」と「難関帯の単語」です。
...
## 【実データ】難関帯の単語は"性格"が分かれる

序盤は共通でも、終盤（難関レベル）になると収録語の傾向に違いが出ます。実際の終盤の単語を比べると——

| 単語帳 | 終盤に多い単語の例 | 傾向 |
|---|---|---|
| システム英単語（Final Stage） | aesthetic（美的）／intrinsic（本来の）／manifest（明白な）／brutal（残忍な）／renowned（有名な） | 抽象的な形容詞が多い |
| ターゲット1900（Part 3） | mortgage（住宅ローン）／attorney（弁護士）／erosion（浸食）／archaeology（考古学）／subsidy（補助金） | 専門・学術的な名詞が多い |

シス単の終盤は「論説文で出る抽象語・形容詞」、ターゲットの終盤は「テーマ性のある専門名詞」に寄る傾向があります。とはいえ **eloquent（雄弁な）・dumb など、両方に収録される難語も多数**あり、最難関レベルでも共通部分は残ります。
...
## 【実データ】LEAP最大の特徴は「テーマ別」配列

ターゲットやシス単が「頻度順（でる順）」で並ぶのに対し、LEAPは **意味のまとまり（テーマ）ごと** に単語を配置します。実際にLEAPの冒頭を見ると、頻出の基本動詞がバラバラに並ぶのではなく、次のように「関連する意味の単語」がまとまって登場します。
...
関連語をまとめて覚えられるため、**単語同士がつながって記憶に残りやすく**、英作文やスピーキングで「使い分け」がしやすくなります。頻度順にはない、LEAPならではの強みです。
