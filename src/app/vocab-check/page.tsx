...
const FAQ_ITEMS = [
  ...
  {
    // Codexレビュー指摘対応: ログイン状態ではVocabCheckRunnerが設問ごとの正誤を
    // vocab_check_answer_submittedイベントとして送信し、/api/analytics/eventsが
    // 現在のSupabase user_idを付与して保存する。「匿名で実行される」という記述は
    // 未ログイン時のみ正しく、ログイン時の実態と異なっていたため、両者を分けて
    // 正確に説明するよう修正した。
    q: "回答データはどのように扱われますか？",
    a: "未ログインの場合は匿名で診断が実行されます。ログインした状態で診断する場合は、設問ごとの正誤を含む学習データがご自身のアカウントに紐づけて記録されます(他の学習機能の分析に使われるのと同様の扱いです)。いずれの場合も、個々の回答内容を第三者に販売・提供することはありません。学習データの取り扱い全般については、プライバシーポリシーをご確認ください。",
  },
  {
