...
export async function generateMetadata(
  ...
  return {
    ...
    description: `${entry.word}（${entry.meaningJa}）の意味・例文・ニュアンス・覚え方・語源・英作文での使い方をまとめて解説。${entry.examLevels.join("・")}で頻出の単語です。`,
...
export default async function WordPage({ params }: { params: Promise<{ word: string }> }) {
  ...
  const qaBlocks = getWordQaBlocks(entry);
  ...
  // 本文に実際に表示するQ&Aブロックとまったく同じ内容だけをFAQPageに載せる
  // (JSON-LDと可視本文の乖離を避けるため)。リッチリザルトが必ず出るとは限らない。
  const faqLd = {
    "@context": "https://schema.org",
    "@type": "FAQPage",
    ...
    <AppShell>
      ...
      <article>
        ...
        <header className="mt-3">
          ...
          {/* 冒頭の1文結論(AEO向け): 検索エンジン・回答エンジンが最初に拾いやすい位置に配置 */}
          <p className="text-base font-semibold text-navy-800 mt-2 leading-relaxed">{entry.conclusion}</p>
        ...
        <section className="mt-5 bg-white rounded-2xl border border-navy-100 p-5">
        ...
        <section className="mt-3 bg-white rounded-2xl border border-navy-100 p-5">
        ...
        <section className="mt-3 bg-white rounded-2xl border border-navy-100 p-5">
          <h2 className="text-sm font-bold text-navy-800 mb-2">小論文・英作文での使い方</h2>
          <p className="text-sm text-navy-700 leading-relaxed">{entry.essayUsage}</p>
        </section>

        <section className="mt-3 bg-amber-50 border border-amber-100 rounded-2xl p-5">
          <h2 className="text-sm font-bold text-navy-800 mb-2">よくある間違い</h2>
          <p className="text-sm text-navy-700 leading-relaxed">{entry.commonMistake}</p>
        </section>

        <section className="mt-3 bg-navy-50 border border-navy-100 rounded-2xl p-5">
          <h2 className="text-sm font-bold text-navy-800 mb-2">試験での出やすさ</h2>
          <p className="text-sm text-navy-700 leading-relaxed">{entry.examFrequency}</p>
        </section>
        ...
        {/* AEO向け「問い→答え」ブロック。上記の各セクションと同じ内容から導出しており、
            JSON-LDのFAQPageと可視本文が一致するようにしている。 */}
        <section className="mt-3 bg-white rounded-2xl border border-navy-100 p-5">
          <h2 className="text-sm font-bold text-navy-800 mb-3">よくある質問</h2>
          <div className="space-y-3">
            {qaBlocks.map((qa) => (
              <section key={qa.q}>
                <h3 className="text-sm font-bold text-navy-800">{qa.q}</h3>
                <p className="text-sm text-navy-700 leading-relaxed mt-1">{qa.a}</p>
              </section>
            ))}
          </div>
        </section>
        ...
        <p className="text-[11px] text-navy-400 mt-4">
          ※ 例文は許諾済みの教材データに基づきます。ニュアンス・覚え方・語源・英作文での使い方・よくある間違いの解説はLoop Vocabularyが独自に作成したものです。
        </p>
      </article>
    </AppShell>
  );
